...
const { callAI, parseJSONResponse, AI_AVAILABLE, mockCategorise } = require('../utils/ai');
...
router.post('/scan-receipt', protect, upload.single('receipt'), async (req, res) => {
  ...
  // Live mode — call the configured AI provider (Gemini preferred, Claude fallback) with vision
  ...
  const text = await callAI({
    ...
    prompt: `Analyse this receipt image and extract the following information. Respond ONLY with a JSON object, no other text:
    {
  ...
  let data;
    try {
      ...
      console.error('Failed to parse receipt response:', text);
...
router.post('/categorise', protect, async (req, res) => {
  ...
  const text = await callAI({
    max_tokens: 200,
    prompt: `Categorise this expense description into exactly one category. Respond ONLY with a JSON object, no other text:
...
router.post('/categorise-bulk', protect, async (req, res) => {
  ...
  const descList = descriptions.map((d, i) => `${i + 1}. "${d}"`).join('\n');
    const text = await callAI({
      max_tokens: 1000,
      prompt: `Categorise each of these expense descriptions. Respond ONLY with a JSON array, no other text:
